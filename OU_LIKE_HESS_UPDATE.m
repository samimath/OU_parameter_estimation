%% function hess = OU_LIKE_HESS_UPDATE(lambda,mu,sigma2,u,v,X)
...
function [update] = OU_LIKE_HESS_UPDATE(lambda,mu,sigma2,u,v,X)
...
h_lmblmb_2(j) = ((eta(j))^2*eta_explmb(j)*(1+(eta_explmb(j))^2)*...
    (X(:,j-1)')*B_inv*X_q(:,j))/((eta_explmb2(j))^2);
h_lmblmb_3(j) = (eta(j)*eta_explmb(j))^2*(X(:,j-1)')*...
    B_inv*X(:,j-1)/(eta_explmb2(j));
h_lmblmb_4(j) = 2*(eta(j)*eta_explmb(j))^2*(1+(eta_explmb(j))^2)*...
    X_q(:,j)'*B_inv*X_q(:,j)/((eta_explmb2(j))^3);
h_lmblmb_5(j) = 2*(eta(j)*eta_explmb(j))^2*eta_explmb(j)*...
    (X(:,j-1)')*B_inv*X_q(:,j)/((eta_explmb2(j))^2);
% split the H_12 = h_lambdamu:
h_lmbmu_1(j) = (eta(j)*eta_explmb(j)*X(:,j-1)'*...
    OU_DBINV(mu,v)*X_q(:,j))/(eta_explmb2(j));
h_lmbmu_2(j) = ((eta(j)*(eta_explmb(j))^2)*X_q(:,j)'*...
    OU_DBINV(mu,v)*X_q(:,j))/((eta_explmb2(j))^2);
h_mulmb_1(j) = (eta(j)*((eta_explmb(j))^2)*X(:,j-1)'*...
    OU_DBINV(mu,v)*X_q(:,j))/(eta_explmb2(j));
h_mulmb_2(j) = (eta(j)*((eta_explmb(j))^2)*X_q(:,j)'*...
    OU_DBINV(mu,v)*X_q(:,j));
% define the lambda term in h_mumu:
h_mumu_star(j) = (X_q(:,j)'*OU_D2BINV(mu,v)*X_q(:,j))/(eta_explmb2(j));
dl_mu_star(j) = (X_q(:,j)'*OU_DBINV(mu,v)*X_q(:,j))/(eta_explmb2(j));
% define the terms in the first derivative of l(theta):
dl_lmb_1(j)=(2*eta(j)*(eta_explmb(j))^2)/(eta_explmb2(j));
dl_lmb_2(j)=...
    (eta(j)*eta_explmb(j)*X(:,j-1)'*B_inv*X_q(:,j))/(eta_explmb2(j));
dl_lmb_3(j)=...
    eta(j)*(eta_explmb(j)/(eta_explmb2(j)))^2*((X_q(:,j)')*B_inv*X_q(:,j));          
end 
for k=2:N
...
h_lmblmb = sum(N*h_lmblmb_1(2:M))-...
           (2/sigma2)*(sum(h_lmblmb_2(2:M)-h_lmblmb_3(2:M)-...
           h_lmblmb_4(2:M)+h_lmblmb_5(2:M)));
h_lmbmu=(2/sigma2)*sum(h_lmbmu_1(2:M)-h_lmbmu_2(2:M));
h_mulmb = (2/sigma2)*(sum(h_mulmb_1(2:M)-h_mulmb_2(2:M)));
h_mumu = sum(M*h_mumu_1(2:N)) +...
       (1/sigma2)*(X(:,1)'*OU_D2BINV(mu,v)*X(:,1) +...
       sum(h_mumu_star(2:M)));
% Define the Hessian matrix
H=[h_lmblmb,h_lmbmu; h_mulmb h_mumu];
% Partial derivatives:
dl_lmb=sum(N*dl_lmb_1(2:M)+(2/sigma2)*...
    dl_lmb_2(2:M)-(2/sigma2)*dl_lmb_3(2:M));
dl_mu = sum(M*dl_mu_1(2:N))+(1/sigma2)*(X(:,1)'*...
    OU_DBINV(mu,v)*X(:,1)+sum(dl_mu_star(2:M)));
update = H\[dl_lmb;dl_mu];
